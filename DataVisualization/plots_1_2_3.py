...
def make_plots_1(db, team_1, team_2 = "All"):
    ...
    aus_stadiums = ['Sydney Cricket Ground', 'Brisbane Cricket Ground, Woolloongabba', 'Melbourne Cricket Ground', 'GMHBA Stadium, South Geelong, Victoria', 'Perth Stadium', 'Manuka Oval, Canberra', 'Bellerive Oval', 'Brisbane Cricket Ground, Woolloongabba, Brisbane', 'Manuka Oval', 'Carrara Oval', 'Adelaide Oval']
    ref_db_win_loss = ref_db.loc[:, ["id", "year", "venue", "team_A", "team_B", "winner"]]
    ref_db_win_loss["In_Australia"]=ref_db_win_loss["venue"].isin(aus_stadiums)
    ref_db_win_loss = ref_db_win_loss.drop(columns='venue')
    ...
    for year_ref in years:
        ...
        try:
            ...
            win_loss_ball = 0
        win_loss_record.append((year_ref, np.round((100*num_wins_total/total_matches),2), win_loss_bat, win_loss_ball))
...
def make_plots_2(db, team_1, team_2 = "All"):
    ...
    aus_stadiums = ['Sydney Cricket Ground', 'Brisbane Cricket Ground, Woolloongabba', 'Melbourne Cricket Ground', 'GMHBA Stadium, South Geelong, Victoria', 'Perth Stadium', 'Manuka Oval, Canberra', 'Bellerive Oval', 'Brisbane Cricket Ground, Woolloongabba, Brisbane', 'Manuka Oval', 'Carrara Oval', 'Adelaide Oval']
    ref_db_avg_score = db.loc[:, ["id", "year", "venue", "team_A", "team_B", "Total_Score_A", "Total_Wicket_A"]]
    ref_db_avg_score["In_Australia"]=ref_db_avg_score["venue"].isin(aus_stadiums)
    ref_db_avg_score = ref_db_avg_score.drop(["venue"], axis = 1)
...
def make_plots_3(db, team_1, team_2 = "All"):
    """
    This function plots the average runs scored and average wickets conceded per phase of the match (for three scenarios - batting first, batting second, and overall) for any of the top 12 teams of T20WC 2022. 
    These plots can be generated either for all T20I matches against all other teams or against a particular team.  
    ...
    aus_stadiums = ['Sydney Cricket Ground', 'Brisbane Cricket Ground, Woolloongabba', 'Melbourne Cricket Ground', 'GMHBA Stadium, South Geelong, Victoria', 'Perth Stadium', 'Manuka Oval, Canberra', 'Bellerive Oval', 'Brisbane Cricket Ground, Woolloongabba, Brisbane', 'Manuka Oval', 'Carrara Oval', 'Adelaide Oval']
    ref_db_phase_score = db.loc[:, ["id", "year", "venue", "team_A", "team_B", "Runs_in_Powerplay", "Wickets_lost_in_Powerplay", "Runs_in_middle_overs", "Wickets_lost_in_middle_overs", "Runs_in_Death_overs", "Wickets_lost_in_death_overs"]]
    ref_db_phase_score["In_Australia"]=ref_db_phase_score["venue"].isin(aus_stadiums)
    ref_db_phase_score = ref_db_phase_score.drop(["venue"], axis = 1)
    ...
    for year_ref in years:
        ...
        try: 
            avg_runs_P_overall = np.round((num_score_P_overall/total_matches_overall),2)
        except:
        ...
        try: 
            avg_runs_M_overall = np.round((num_score_M_overall/total_matches_overall),2)
        except:
        ...
        try: 
            avg_runs_D_overall = np.round((num_score_D_overall/total_matches_overall),2)
        except:
        ...
        try: 
            avg_runs_P_bat = np.round((num_score_P_bat/total_matches_bat),2)
        except:
            avg_runs_P_bat = 0
        try: 
            ...
            avg_wickets_P_bat = 0
        try: 
            avg_runs_M_bat = np.round((num_score_M_bat/total_matches_bat),2)
        except:
            avg_runs_M_bat = 0
        try: 
            ...
            avg_wickets_M_bat = 0
        try: 
            avg_runs_D_bat = np.round((num_score_D_bat/total_matches_bat),2)
        except:
            avg_runs_D_bat = 0
        try: 
        ...
        try: 
            avg_runs_P_ball = np.round((num_score_P_ball/total_matches_ball),2)
        except:
            avg_runs_P_ball = 0
        try: 
        ...
        try: 
            avg_runs_M_ball = np.round((num_score_M_ball/total_matches_ball),2)
        except:
            avg_runs_M_ball = 0
        try: 
        ...
        try: 
            avg_runs_D_ball = np.round((num_score_D_ball/total_matches_ball),2)
        except:
            avg_runs_D_ball = 0
        try: 
    ...
    ax[0].bar(r, phase_score_df.iloc[:,0], width=width, label = 'Powerplay', color = 'b')
    ax[0].bar(r+width, phase_score_df.iloc[:,2], width=width, label = 'Middle Overs', color = 'g')
    ax[0].bar(r+(2*width), phase_score_df.iloc[:,4], width=width, label = 'Death Overs', color = 'r')
    if team_2 == "All":
        ax[0].set_title('Phase wise breakdown of avg. runs scored across all T20 matches since 2016: '+str(team_1), loc='center')
    else:
        ax[0].set_title('Phase wise breakdown of avg. runs scored against '+str(team_2)+' in all T20I matches since 2016: '+str(team_1), loc='center')
    ax[0].set_xlabel('Year')
    ax[0].set_ylabel('Avg. runs scored')
    ...
    ax[1].bar(r, phase_score_df.iloc[:,1], width=width, label = 'Powerplay', color = 'b')
    ax[1].bar(r+width, phase_score_df.iloc[:,3], width=width, label = 'Middle Overs', color = 'g')
    ax[1].bar(r+(2*width), phase_score_df.iloc[:,5], width=width, label = 'Death Overs', color = 'r')
    if team_2 == "All":
    ...
    ax[0].bar(r, phase_score_df.iloc[:,6], width=width, label = 'Powerplay', color = 'b')
    ax[0].bar(r+width, phase_score_df.iloc[:,8], width=width, label = 'Middle Overs', color = 'g')
    ax[0].bar(r+(2*width), phase_score_df.iloc[:,10], width=width, label = 'Death Overs', color = 'r')
    if team_2 == "All":
        ax[0].set_title('Phase wise breakdown of avg. runs scored across all T20 matches batting first since 2016: '+str(team_1), loc='center')
    else:
        ax[0].set_title('Phase wise breakdown of avg. runs scored against '+str(team_2)+' in all T20I matches batting first since 2016: '+str(team_1), loc='center')
    ax[0].set_xlabel('Year')
    ax[0].set_ylabel('Avg. runs scored')
    ax[0].set_xticks(r+width) 
    ax[0].set_xticklabels(years)
    ...
    ax[1].bar(r, phase_score_df.iloc[:,7], width=width, label = 'Powerplay', color = 'b')
    ax[1].bar(r+width, phase_score_df.iloc[:,9], width=width, label = 'Middle Overs', color = 'g')
    ax[1].bar(r+(2*width), phase_score_df.iloc[:,11], width=width, label = 'Death Overs', color = 'r')
    if team_2 == "All":
    ...
    ax[0].bar(r, phase_score_df.iloc[:,12], width=width, label = 'Powerplay', color = 'b')
    ax[0].bar(r+width, phase_score_df.iloc[:,14], width=width, label = 'Middle Overs', color = 'g')
    ax[0].bar(r+(2*width), phase_score_df.iloc[:,16], width=width, label = 'Death Overs', color = 'r')
    if team_2 == "All":
        ax[0].set_title('Phase wise breakdown of avg. runs scored across all T20 matches batting second since 2016: '+str(team_1), loc='center')
    else:
        ax[0].set_title('Phase wise breakdown of avg. runs scored against '+str(team_2)+' in all T20I matches batting second since 2016: '+str(team_1), loc='center')
    ax[0].set_xlabel('Year')
    ax[0].set_ylabel('Avg. runs scored')
    ax[0].set_xticks(r+width) 
    ax[0].set_xticklabels(years)
    ...
    ax[1].bar(r, phase_score_df.iloc[:,13], width=width, label = 'Powerplay', color = 'b')
    ax[1].bar(r+width, phase_score_df.iloc[:,15], width=width, label = 'Middle Overs', color = 'g')
    ax[1].bar(r+(2*width), phase_score_df.iloc[:,17], width=width, label = 'Death Overs', color = 'r')
    if team_2 == "All":
