...
def batting_bowling_performances(avg_db, utd_db):
    ...
    x = np.arange(len(ds_win_1['opposition'].values.tolist()))  # the label locations
    width = 0.35  # the width of the bars

    fig, ax = plt.subplots()
    fig.set_size_inches(12,5)

    rects1 = ax.bar(x - width/2, ds_win_1['batting_perf'].values.tolist(), width, label='Batting Performance')
    rects2 = ax.bar(x + width/2, ds_win_1['bowling_perf'].values.tolist(), width, label='Bowling Performance')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Number of Good Performances')
    ax.set_title('England\'s Good Performances Against Tournament Teams')
    ax.set_xticks(np.arange(len(x)))


    ax.set_xticklabels(ds_win_1['opposition'].values.tolist())
    ax.legend()
    fig.tight_layout()
