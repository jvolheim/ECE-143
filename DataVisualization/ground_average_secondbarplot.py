...
def batting_bowling_performances(avg_db, utd_db):
    ...
    temp={'home':[],'batting_perf':[],'bowling_perf':[]}
    total=pd.DataFrame(temp)
    for i in final_four:
        ds_win_1 = ds_2[(ds['win_or_loss'] == 1) & (ds['home']==i)]
        total.loc[len(total.index)] = [i, sum(ds_win_1['batting_perf']),sum(ds_win_1['bowling_perf'])]
    
    x = np.arange(len(total['home'].values.tolist()))  # the label locations
    width = 0.35  # the width of the bars

    fig, ax = plt.subplots()
    fig.set_size_inches(12,5)

    rects1 = ax.bar(x - width/2, total['batting_perf'].values.tolist(), width, label='Batting Performance')
    rects2 = ax.bar(x + width/2, total['bowling_perf'].values.tolist(), width, label='Bowling Performance')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Number of Good Performances')
    ax.set_title('Good Performances in a Loss-Final Four Teams')
    ax.set_xticks(np.arange(len(x)))


    ax.set_xticklabels(total['home'].values.tolist())
    ax.legend()
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    batting_bowling_performances(avg_db, uptd_db)
